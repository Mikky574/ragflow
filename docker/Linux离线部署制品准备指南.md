...
目标平台：**Linux x86_64、glibc 2.27、CPython 3.13、NVIDIA GPU**。仓库 `pyproject.toml` 与 `uv.lock` 固定 Python 3.13，Python 3.10 的 wheel 不能混入该制品包。
...
| 层级 | 必需项 | 目标机验收 |
| --- | --- | --- |
| 系统 | Linux x86_64、glibc >= 2.27、bash、tar、pkg-config、libjemalloc2 | `uname -m`、`ldd --version`、`ldconfig -p \| grep jemalloc` |
...
## 2. glibc 2.27 与无 sudo 的边界

你的 glibc 2.27 可以运行当前 `onnxruntime-gpu==1.23.2` 的 CPython 3.13 x86_64 wheel；该 wheel 标记为 `manylinux_2_27_x86_64`。因此 Python + GPU OCR 开发态可行。

无 sudo 不妨碍把 RAGFlow、Python venv、模型和前端文件放在 `$HOME/ragflow`，但不能安装或升级以下宿主机基础设施：Docker Engine、NVIDIA 驱动、NVIDIA Container Toolkit、系统 Python、Node、`libjemalloc2`。它们必须已由管理员安装。先确认 `docker ps` 不报 permission denied；若失败，管理员需把你的账号加入 `docker` 组后重新登录。不要尝试以 root 运行应用，也不要修改系统 glibc。

本仓库的 Go 原生构建资源中包含 glibc 2.28 标记的静态库，它们用于 Go 二进制构建，不属于本次 Python 开发态运行路径。glibc 2.27 主机上不要在本机执行 Go native build；本指南仅部署 Python API、worker、Vite 和容器依赖。
...
## 5. 生成 Linux Python 3.13 wheelhouse
...
### 5.2 在 Linux 容器内下载并构建 wheel

`graspologic` 是 Git 依赖，必须打成 wheel，不能只用 Windows 的 `pip download`。使用 `manylinux2014`（glibc 2.17）构建，生成 wheel 才能兼容 glibc 2.27；不要用 `python:3.13-bookworm`，其编译产物可能依赖更高版本 glibc。若构建容器没有 git，先在联网机把固定提交 clone 到制品目录，再以本地源码构建 graspologic wheel。

```powershell
$PyImage = 'quay.io/pypa/manylinux2014_x86_64'
$Py = '/opt/python/cp313-cp313/bin/python'
docker pull $PyImage
docker run --rm `
  -e HTTP_PROXY -e HTTPS_PROXY -e NO_PROXY `
  -v "${Bundle}:/bundle" -w /bundle $PyImage `
  bash -lc '/opt/python/cp313-cp313/bin/python -m pip install --upgrade pip wheel setuptools && /opt/python/cp313-cp313/bin/python -m pip wheel --wheel-dir /bundle/wheelhouse -r /bundle/requirements-linux-py313.txt'
```
...
docker run --rm `
  -e HTTP_PROXY -e HTTPS_PROXY -e NO_PROXY `
  -v "${Bundle}:/bundle" -w /bundle $PyImage `
  bash -lc '/opt/python/cp313-cp313/bin/python -m venv /tmp/model-venv && /tmp/model-venv/bin/pip install huggingface-hub && /tmp/model-venv/bin/python -c "from huggingface_hub import snapshot_download; snapshot_download(repo_id=\"BAAI/bge-m3\", local_dir=\"/bundle/models/bge-m3\")"'
```
...
上传方式可选 U 盘、内网文件服务器或 `scp`。目标目录假定为 `$HOME/ragflow-offline-bundle`，源代码目录为 `$HOME/ragflow`。
...
```bash
cd $HOME/ragflow-offline-bundle
sha256sum -c SHA256SUMS
cat source-revision.txt
```

解压对应版本的 `ragflow-source.tar.gz` 到 `$HOME/ragflow`。如果使用 Git 仓库，`git rev-parse HEAD` 必须与 `source-revision.txt` 一致。

### 10.2 从 wheelhouse 安装 Python 依赖

```bash
cd $HOME/ragflow
python3.13 -m venv .venv
.venv/bin/pip install --no-index --find-links $HOME/ragflow-offline-bundle/wheelhouse --no-deps $HOME/ragflow-offline-bundle/wheelhouse/*.whl
...
cp -a $HOME/ragflow-offline-bundle/models/ragflow_deps/. ragflow_deps/
mkdir -p .cache/models
cp -a $HOME/ragflow-offline-bundle/models/bge-m3 .cache/models/
tar -C web -xzf $HOME/ragflow-offline-bundle/frontend/node_modules-linux-x64.tar.gz
```

### 10.4 导入镜像并启动容器依赖

```bash
docker load --input $HOME/ragflow-offline-bundle/docker/ragflow-images.tar
cd $HOME/ragflow
