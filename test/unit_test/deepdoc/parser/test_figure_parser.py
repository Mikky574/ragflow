...
@pytest.mark.p1
def test_pdf_figure_wrapper_reports_missing_vision_model(monkeypatch):
    module, FakeImage = _load_figure_parser(monkeypatch)
    callback = Mock()
    figure = ((FakeImage(), ["Figure 1."]), [(0, 10, 100, 20, 200)])
    module.get_tenant_default_model_by_type = Mock(side_effect=LookupError("Vision model is not configured"))
    ...
    result = module.vision_figure_parser_pdf_wrapper(
        ...
        callback=callback,
        tenant_id="tenant-id",
        parser_config={"image_vision_enable": True},
    )

    assert result == [figure]
    callback.assert_called_once_with(0.7, "Figure vision enhancement skipped: configure a default Vision model to extract image content.")


@pytest.mark.p1
def test_pdf_figure_wrapper_honors_explicit_disable(monkeypatch):
    module, FakeImage = _load_figure_parser(monkeypatch)
    callback = Mock()
    figure = ((FakeImage(), ["Figure 1."]), [(0, 10, 100, 20, 200)])
    model_lookup = Mock()
    module.get_tenant_default_model_by_type = model_lookup
    ...
    result = module.vision_figure_parser_pdf_wrapper(
        ...
        callback=callback,
        tenant_id="tenant-id",
        parser_config={"image_vision_enable": False},
    )

    assert result == [figure]
    model_lookup.assert_not_called()
    callback.assert_called_once_with(0.7, "Figure vision enhancement is disabled by the parser configuration.")
